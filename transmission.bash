#!/bin/bash
# TODO description write up

function onExitSignal {
  log 'received shutdown signal, deleting output files'
  rm -f ${outputDir}/transmission.*    # file read by conky
...
activeTorrentsFile=${outputDir}/transmission.activeTorrents
...
while [ true ]; do
  ...
  transmission-remote -t active -l | grep -E '(Seeding|Downloading)' | sed 's/.\+\(Seeding\|Downloading\) \+//' | sed 's/^/${voffset 3}${offset 5}/' | sed 's/#/\\#/g' | sort > ${activeTorrentsFile}
  ...
  transmission-remote -t active -pi | grep -e '^[0-9]' | cut -c 1-15,78-100 --output-delimiter='  ' | sort -t . -k 1n -k 2n -k 3n -k 4n | sed 's/^/${voffset 3}${offset 5}/' | head -${maxLines} > ${peersFile}
  sleep 3s &  # run the sleep process in the background so we can kill it if we get a termination signal
